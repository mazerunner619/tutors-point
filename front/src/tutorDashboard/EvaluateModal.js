...
export default function Evaluate({ assignment, onHide, show, userid }) {
  ...
  async function HandleClick(e) {
    ...
    const { data } = await axios.post(
      `/asn/${userid}/assignment/${assignment.AID}/evaluate/${assignment.SID}`,
      evaluate
    );
  ...
  function handleChange(e) {
    ...
    if (name === "score") if (value > 10) value = 10;
    if (value < 0) value = 0;
    setEvaluate({
      ...evaluate,
      [name]: value,
    ...
    >
      ...
      <Modal.Body>
        <center>
          ...
          />
          <br />
          <a href={assignment.assignment} target="_blank" rel="noreferrer">
            open assignment
          </a>
          <hr />
        </center>
        <Form style={{ width: "100%" }}>
          <Form.Group controlId="exampleForm.ControlTextarea1">
            ...
            <Form.Control
              as="textarea"
              rows="3"
              placeholder="any remarks for the assignment"
              required
              value={evaluate.remarks}
      ...
      <Modal.Footer>
        <Button
          block
          disabled={saving}
          style={{ marginTop: "2%", width: "100%", borderRadius: "20px" }}
          variant="success"
          onClick={HandleClick}
        >
          {saving ? "Saving..." : "Save"}
